...
## 出力単位について（単一ファイル／ファイルごと）

複数ファイルを読み込んで OCR を実行した場合、保存時に出力単位を選択できます
（入力ファイルが 1 つだけの場合はこの設定は結果に影響しません）：

- **単一ファイル**: すべてのページの結果を 1 つのファイルにまとめて保存（従来の動作）
- **ファイルごと**: 入力ファイルごとに個別のファイルとして保存します。
  出力ファイル名は元の入力ファイル名と同一になります

「ファイルごと」を選択した場合、保存ボタンを押すと保存先フォルダの選択ダイアログが
表示されるだけで、ファイル名の入力は不要です。同名ファイルが重複する場合は自動的に
連番（`_2`, `_3` …）が付与されます。
