...
if (-not $pyOk) {
    ...
    Write-Host "Python 3.11.13 をダウンロード中..."
    $downloadOk = $true
    try {
        ...
        Write-Host "  ファイル: python-3.11.13-amd64.exe"
        Write-Host ""
    }
...
if (-not (Test-Path $HfCli)) {
    Write-Host "[エラー] huggingface-cli が見つかりません。Step 3 を確認してください。" -ForegroundColor Red
} else {
